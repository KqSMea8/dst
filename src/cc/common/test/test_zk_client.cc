...
void* registe_routine(void* arg) {
    ...
    node.data.buf = reinterpret_cast<char*>(&foo);
    node.data.len = sizeof(foo);
    node.path.buf = static_cast<char*>(calloc(1, kBufferLen));
    node.path.len = kBufferLen;
    if (NULL == node.path.buf)  {
    ...
    for (int32_t i = 0; i < g_node_num; i++) {
        ...
        printf("[ZKCLIENT_TESTER] create ephemeral node[%d]@[%s][%s] ...OK\n",
               i, node.path.buf, zk_servers_list);
        /* copy node's real path for deletion in later */
        paths[i] = node.path.buf;
    }
    ...
    /* test data getter */
    Foo* pfoo;
    memset(node.data.buf, 0, node.data.len);
    ret = zk_client.GetNodeData(node.path.buf, &node.data);
    if (ret < error::OK) {
        printf("[ZKCLIENT_TESTER] fail to get data@node[%s]\n", node.path.buf);
        goto ERR_RET;
    }
    pfoo = reinterpret_cast<Foo*>(node.data.buf);
    printf("[ZKCLIENT_TESTER] get data[a=%d, b=%ld, c=%c%c]@node[%s] ...OK\n",
           pfoo->a, pfoo->b, pfoo->c[0], pfoo->c[1], node.path.buf);

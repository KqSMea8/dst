...
*****************************************************************/
/**
 * @file multi_key_joiner.h
 * @author lanbijia
 * @date 2012/11/5
 * @brief 定义MultiJoiner类，实现Join操作
...
#ifndef __DSTREAM_GENERIC_OPERATOR_MULTI_KEY_JOINER_H__ // NOLINT
#define __DSTREAM_GENERIC_OPERATOR_MULTI_KEY_JOINER_H__ // NOLINT
...
public:
     ...
     * @param join_tuples [out] 如果join成功，返回join成功的tuples（可能返回多个tuple，多对多join）
     * @return ErrorCode 返回join的结果
     *         error::OK 成功
     *         error::UNINIT 未初始化
     */
    virtual ErrorCode Join(const KeyType& keys,
            bool is_reserve, std::vector<DataType>* join_values);
...
protected:
    typedef MultiJoinTable<KeyType, DataType> KVTable;
    typedef boost::shared_ptr<KVTable> KVTablePtr;
    typedef MultiJoinTableIterator<KeyType, DataType> KVTableIterator;

...
SingleKeyJoiner::SingleKeyJoiner()
        : m_t_table(NULL),
        ...
        m_join_with_store(SingleKeyJoiner::kDefaultJoinWithStore),
        m_restore_from_store(SingleKeyJoiner::kDefaultRestoreFromStore),
        m_ignore_store_error(SingleKeyJoiner::kDefaultIgnoreStoreError),
        m_batch_insert(SingleKeyJoiner::kDefaultStoreBatchInsert),
        m_batch_count(SingleKeyJoiner::kDefaultStoreBatchCount),
        m_batch_interval_in_ms(SingleKeyJoiner::kDefaultStoreBatchIntervalMs) {
...
bool SingleKeyJoiner::CheckAndReadConfig(const dstream::config::Config& conf,
        const std::string& id) {
    uint32_t fail_cnt = 0;
...
SingleKeyJoiner::ErrorCode SingleKeyJoiner::Initialize(const dstream::config::Config& conf,
        const std::string& id,
...
SingleKeyJoiner::ErrorCode SingleKeyJoiner::Init(const dstream::config::Config& conf,
        const std::string& id,
        const std::string& name,
    ...
    if (time_table) {
        int ret = 0;
        if (0 != (ret = time_table->Init(m_step_in_sec, m_window_size_by_step))) {
            JOINER_ERROR("[%s] fail to init TimeWindow, err(%d).", __FUNCTION__, ret);
            return error::JOINER_WINDOW_INIT_ERROR;
        }
        // 是否使用外部存储
        if (m_join_with_store && NULL != store_accessor) {
            ret = time_table->SetStore(store_accessor,
                    id, m_restore_from_store, m_ignore_store_error, storage::HASH_TABLE);
            if (0 != ret) {
...
SingleKeyJoiner::ErrorCode SingleKeyJoiner::Join(const KeyType& keys,
        bool is_reserve,
        DataType* join_value) {
    ...
    for (TimedSlidingWindowsIterator<KVTable> iter(*m_t_table);
        ...
        // 单值join，每次返回至多一个结果
        DataType tmp_join_value;
        ret = t->Find(keys, &tmp_join_value);
        if (ret <= 0) { // find error (ret < 0) ||result is none (ret == 0)
            if (ret < 0) {
...
SingleKeyJoiner::ErrorCode SingleKeyJoiner::Record(uint32_t tuple_time,
        bool replace,
        const KeyType& keys,
    ...
    // 向数据块中插入数据
    // replace为false时如果已经存在key则插入失败返回-1
    ret = t->Insert(keys, value, replace);
    if (0 != ret) {
...
int SingleKeyJoiner::OnWindowCreatingCallback(SingleKeyJoiner::KVTablePtr window_data,
        uint32_t ts,
    ...
    SingleKeyJoiner* joiner_this = reinterpret_cast<SingleKeyJoiner*>(context);
    if (NULL == window_data) {
...
int SingleKeyJoiner::OnWindowSlidingOutCallback(SingleKeyJoiner::KVTablePtr window_data,
        uint32_t ts,
        void* context) {
    int ret = 0;
...
void SingleKeyJoiner::RegisterDataTimeOutCallBack(DataTimeOutCallBack call_back, void* context) {

...
void JoinerTask::init() {
    ...
    imp1_joiner_ = new (std::nothrow) dstream::generic_operator::MultiKeyJoiner();
...
void JoinerTask::onTuples(const Tuples& tuples) {
    for (size_t i = 0; i < tuples.size(); i++) {
        ...
        if (tuple->tag() == "importer0") {
            ...
            err_code = imp1_joiner_->Join(join_key_str, false,
                                          &join_data_list); // ×ö²éÕÒ£¬ÕÒµ½ºóÉ¾³ýÊý¾Ý(multi_hash_table)
            if (dstream::generic_operator::error::OK == err_code) {
                ...
                for (iter = join_data_list.begin(); iter != join_data_list.end(); ++iter) {
                    ...
                    if (!importer1.ParseFromString(*iter)) {
            ...
            if (dstream::generic_operator::error::OK == err_code) {
                ...
                err_code = imp1_joiner_->Record(tuple_time, false, join_key_str,
                                                join_data); // multi_hash_table,²»Ìæ»»£¬±£Áôµ¥key¶àvalue
                if (err_code < dstream::generic_operator::error::OK) {
                    DSTREAM_WARN("[%s]imp1 record error,time(%u),key(%s)", __FUNCTION__, tuple_time,
                                 join_key_str.c_str());
                    if (err_code == dstream::generic_operator::error::JOINER_GET_WINDOW_FAIL) {
                        DSTREAM_WARN("[%s]send too old importer1_tuple,time(%u),key(%s)", __FUNCTION__, tuple_time,
                                     join_key_str.c_str());
                    }
                }
            } else {

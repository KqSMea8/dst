...
void JoinerTask::init() {
    // new everything
    imp0_joiner_ = new (std::nothrow) dstream::generic_operator::SingleKeyJoiner();
    ...
    for (size_t i = 0; i < tuples.size(); i++) {
        ...
        if (tuple->tag() == "importer0") {
            ...
            if (dstream::generic_operator::error::OK == err_code) {
                ...
                err_code = imp0_joiner_->Record(tuple_time, true, join_key_str,
            ...
            string join_data;
            err_code = imp0_joiner_->Join(join_key_str, true,
                                          &join_data); // ×ö²é±í£¬ÕÒµ½ºó²»É¾³ýÊý¾Ý(single_hash_table)
            if (dstream::generic_operator::error::OK == err_code) {
                DSTREAM_INFO("[%s][MARK]recv importer1 && join success", __FUNCTION__);
                ImporterTuple importer0;
                if (!importer0.ParseFromString(join_data)) {
                    DSTREAM_ERROR("[%s]fail to parse importer1", __FUNCTION__);
                } else {
